...
const MapboxDrawing: React.FC<MapboxDrawingProps> = ({ geoData }) => {
    ...
    useEffect(() => {
        mapboxgl.accessToken = 'pk.eyJ1IjoiYm9rc2IxIiwiYSI6ImNrcTVjaWZwNzEyMzYycG12bDlud2N4dzgifQ.x9WaaDhFembOygQa1gJTcg';
    ...
    const getBounds = (data: GeoJSON.FeatureCollection): mapboxgl.LngLatBounds | null => {
    ...
    return (
        <div style={{ width: '100%', height: '100%', position: 'relative' }}>
            ...
            <div style={controlContainerStyle}>
                <button onClick={() => handleModeChange('simple_select')} className={buttonStyle}>Select</button>
                <button onClick={() => handleModeChange('draw_point')} className={buttonStyle}>Draw Point</button>
                <button onClick={() => handleModeChange('draw_line_string')} className={buttonStyle}>Draw Line</button>
                <button onClick={() => handleModeChange('draw_polygon')} className={buttonStyle}>Draw Polygon</button>
                <button onClick={() => handleModeChange('direct_select')} className={buttonStyle}>Edit</button>
                <button onClick={handleSave} className={buttonStyle}>Save Changes</button>
            </div>
        </div>
    );
};

const buttonStyle = "bg-blue-100 text-yellow-800 text-xs font-medium me-2 px-3 py-2 rounded dark:bg-yellow-900 dark:text-yellow-300"
